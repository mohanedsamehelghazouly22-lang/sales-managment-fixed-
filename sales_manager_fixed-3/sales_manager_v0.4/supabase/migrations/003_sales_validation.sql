-- v0.4 validation improvements for create_sale.
-- Replaces the RPC with stricter payment/customer validation while preserving
-- the atomic stock update behavior from 002_atomic_sale.sql.
...
begin
  ...
  if p_discount < 0 or p_tax < 0 or p_paid < 0 then
    raise exception 'Invalid money value';
  end if;

  if p_payment_method not in ('cash','card','credit','transfer') then
  ...
  total := greatest(0, round(subtotal - p_discount + p_tax, 2));

  if p_payment_method = 'credit' then
    p_paid := 0;
  elsif p_paid > total then
  ...
  insert into public.sales(
    ...
    p_business_id, p_customer_id, uid, subtotal, p_discount, p_tax,
    total, p_paid, p_payment_method, 'completed', p_note
